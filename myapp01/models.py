...
class HotBoard(models.Model):
    rank = models.IntegerField(verbose_name="排名", help_text="热点在榜单中的位置", db_index=True)
    ...
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="抓取时间", help_text="数据抓取时的系统时间", db_index=True)
...
class Comment(models.Model):
    ...
    comment_date = models.DateTimeField(verbose_name="评论时间", help_text="评论发表的具体时间", db_index=True)
    content = models.TextField(verbose_name="评论内容", help_text="用户发表的评论正文")
    like_count = models.IntegerField(default=0, verbose_name="点赞数", help_text="该评论获得的点赞数量", db_index=True)
